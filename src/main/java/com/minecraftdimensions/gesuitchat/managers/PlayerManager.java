...
public class PlayerManager {
	...
	private static HashMap<String, GSPlayer> onlinePlayers = new HashMap<>();


	public static void addPlayer(GSPlayer player) {
		onlinePlayers.put(player.getName(), player);
		player.updateDisplayName();
	}

	public static Collection<GSPlayer> getOnlinePlayers() {
	...
	public static void setPlayerAFK(Player sender) {
		...
		try {
			...
			out.writeBoolean(sender.hasPermission("gesuit.chat.command.afk.global"));
			out.writeBoolean(sender.hasPermission("gesuit.chat.command.afk.display"));
		} catch (IOException s) {
			s.printStackTrace();
		}
	...
	public static void sendVersion() {
		...
		try {
			...
			out.writeUTF(ChatColor.RED + "Chat - " + ChatColor.GOLD + geSuitChat.instance.getDescription().getVersion());
		} catch (IOException s) {
			s.printStackTrace();
		}
		new PluginMessageTask(b).runTaskAsynchronously(geSuitChat.instance);
