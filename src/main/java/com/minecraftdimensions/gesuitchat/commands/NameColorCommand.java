	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if (args.length == 1) {
			...
			if (sender.getName().equals(FormatUtils.stripAll(nickname))) {
				if (!sender.hasPermission("gesuit.chat.command.namecolor.magic"))
					nickname = FormatUtils.stripMagic(nickname);
				if (!sender.hasPermission("gesuit.chat.command.namecolor.format"))
		...
		if (args.length == 2) {
			if (!sender.hasPermission("gesuit.chat.command.namecolor.other")) {
				...
				if (targetPlayer.equals(FormatUtils.stripAll(nickname))) {
					if (!sender.hasPermission("gesuit.chat.command.namecolor.other.magic"))
						nickname = FormatUtils.stripMagic(nickname);
					if (!sender.hasPermission("gesuit.chat.command.namecolor.other.format"))
						nickname = FormatUtils.stripFormat(nickname);
